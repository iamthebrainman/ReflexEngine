...
def get_readable_time(ts: float = None) -> str:
    dt = datetime.fromtimestamp(ts if ts else time.time())
...
def fibonacci_semantic_spiral(self, text: str, max_k: int = 89) -> List["ThoughtNode"]:
    ...
    with self.lock:
        scored = sorted(
            self.nodes.values(),
            key=lambda n: cos(n.blended_vec(self.nodes), v),
            reverse=True
        )
    fib = [1, 1]
    while len(fib) < max_k:
...
class ThoughtNode:
    def __init__(self, d: Dict[str, Any]):
        ...
        self.readable_ts = get_readable_time(self.ts)
    ...
    def spiral_score(self, v: List[float], all_nodes: Dict[str, "ThoughtNode"] = None) -> float:
        base_vec = self.blended_vec(all_nodes) if all_nodes else self.vec
        return cos(base_vec, v)

    def to_dict(self) -> Dict[str, Any]:
        return {
            "text": self.text, "vec": self.vec, "role": self.role, "tid": self.tid,
            "uuid": self.uuid, "ts": self.ts, "readable_ts": self.readable_ts,
            "links": self.links, "z": self.z, "origin": self.origin
        }
...
class MemoryCrystal:
    ...
    def excite(self, text: str, k: int = 12) -> List[ThoughtNode]:
        ...
        with self.lock:
            scored = sorted(
                self.nodes.values(),
                key=lambda n: cos(n.blended_vec(self.nodes), v) + (1.0 if text_lower in n.text.lower() else 0.0),
                reverse=True
            )
            return scored[:k]
    ...
    def weave(self, tid: str, th: float = 0.3) -> None:
        with self.lock:
            ...
            for a, b in itertools.combinations(nodes, 2):
                if a.uuid != b.uuid and cos(a.blended_vec(self.nodes), b.vec) > th:
            ...
            for node in frontier:
                ...
                for other in self.nodes.values():
                    if other.uuid in seen:
                        continue
                    score = cos(node.blended_vec(self.nodes), other.vec)
                    if score > th:
...
class RollingSTM:
    ...
    def recall(self, k: int = 6) -> str:
        lines = []
        for e in list(self.buf)[-k:]:
            ts = get_readable_time()
...
def _call(model: str, prompt: str, temperature: float = 0.7, max_tokens: int = 512, retries: int = 5, timeout: int = 60) -> str:
    if DEBUG_MODE:
        print("\n--- PROMPT DEBUG ---")
        print(f"Model: {model}")
...
def build_prompt(role: str, user_input: str, dialogue: str, memories: str, extra: str = "") -> str:
    now_ts = now_utc()
    now_str = get_readable_time(now_ts)
    session_start_ts = getattr(build_prompt, "session_start_ts", None)
    if session_start_ts is None:
        session_start_ts = now_ts
        setattr(build_prompt, "session_start_ts", session_start_ts)
    elapsed = int(now_ts - session_start_ts)
    minutes = elapsed // 60
    seconds = elapsed % 60
    ...
    return (
        f"Now: {now_str}\n"
        f"Session started: {get_readable_time(session_start_ts)}\n"
...
def inject_all_lookup_context(tags: List[str], mem: MemoryCrystal, max_words_total: int = 120) -> str:
    ...
    for tag in tags:
        for node in mem.excite(tag, k=6):
            text_line = node.text.strip()
            if text_line and text_line not in seen:
                hits.append((tag, text_line))
                seen.add(text_line)
    summary = []
    word_count = 0
    for tag, text_line in hits:
        words = text_line.split()
        if word_count + len(words) > max_words_total:
            break
        summary.append(f"[LOOKUP:{tag}] {' '.join(words)}")
...
def consciousness_cycle(user: str, stm: RollingSTM, mem: MemoryCrystal, models: ModelSlots) -> Dict[str, Any]:
    tid = str(uuid.uuid4())
    dialogue = stm.recall()
    excited = mem.excite(user, k=8)
    mem_block = "\n".join(f"[MEMORY] {n.text}" for n in excited)
    gut_prompt = build_prompt("Subconscious", user, dialogue, mem_block, "TTask: Identify blind spots or overlooked insights.")
    gut_out = _call(models.subconscious, gut_prompt, temperature=1.0, max_tokens=350)
    lookup_tags = extract_lookups(gut_out)
    lookup_context = inject_all_lookup_context(lookup_tags, mem)
    enriched_mem_block = f"{mem_block}\n{lookup_context}\n[SUBCONSCIOUS INSIGHT]: {gut_out}" if lookup_context else f"{mem_block}\n[SUBCONSCIOUS INSIGHT]: {gut_out}"
    ref_prompt = build_prompt("Conscious", user, dialogue, enriched_mem_block, "Task: Respond. Be brief, but thorough.")
    ...
    if sc_node and cc_node:
        sc_node.links.append(cc_node.uuid)
        cc_node.links.append(sc_node.uuid)
    mem.weave(tid, th=0.25)
    if cc_node:
        activated_nodes = mem.resonance_cascade(cc_node, depth=3, th=0.6)
        for node in activated_nodes:
            if cc_node.uuid not in node.links:
                node.links.append(cc_node.uuid)
    stm.save()
    mem.save()
...
def gui_loop(stm: RollingSTM, mem: MemoryCrystal, models: ModelSlots):
    ...
    entry = ScrolledText(root, wrap=tk.WORD, height=6, width=100)
    entry.pack(padx=10, pady=5, fill=tk.BOTH, expand=True)

    def submit(event=None):
        u = entry.get("1.0", tk.END).strip()
        if not u:
            return
